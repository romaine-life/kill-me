...
| `meal-template` | A meal default (library) | `userId` (`shared`), `templateId`, `name`, `calories`, `proteinGrams`. One set of numbers for one portion — not ingredient-level. Created from the log form ("Save as a default") and updated by "This is the new default"; there is no seed |
| `meal-entry` | A logged meal | `userId`, `date`, `time` (HH:MM, nullable), `templateId` (provenance only, nullable), `name`, `portion`, `calories`, `proteinGrams`, `notes`. Numbers are stored as eaten (already × portion), so changing or deleting a default never rewrites a past day |
